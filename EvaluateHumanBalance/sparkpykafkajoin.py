...
customerSelectStreamingDF = customerRawStreamingDF.selectExpr("cast(key as string) key", "cast(value as string) value") 
...
customerSelectStreamingDF.withColumn("value", from_json("value", redisMessageSchema))\
                .select(col('value.*'))\
...
encodedCustomerStreamingDF = spark.sql("select key, zSetEntries[0].element as encodedCustomer from RedisSortedSet")
...
decodedCustomerStreamingDF = encodedCustomerStreamingDF.withColumn("encodedCustomer",\
    unbase64(encodedCustomerStreamingDF.encodedCustomer).cast("string"))

# TO-DO: parse the JSON in the Customer record and store in a temporary view called CustomerRecords
decodedCustomerStreamingDF.withColumn("encodedCustomer", from_json("encodedCustomer", customerSchema))\
...
# TO-DO: cast the value column in the streaming dataframe as a STRING 
stediEventsStringStreamingDF = stediEventsRawStreamingDF.selectExpr("cast(value as string) value")
...
stediEventsStringStreamingDF.withColumn("value", from_json("value", stediEventSchema))\
                .select(col('value.*'))\
